...
"""
Minimal Python DeepAgent entrypoint example.

This script is meant to be run by Harness-Evaluation through a runner profile
that supplies:

- --task-json <path to rendered task.json>
- --workspace-dir <isolated workspace directory>

It supports two modes:

- fallback: write the expected result file without calling deepagents
- real: call create_deep_agent(...)

For real mode, provide:

- OPENAI_API_KEY in the process environment
- DEEPAGENT_MODEL, for example "openai:gpt-5.4-nano"
"""
...
def _write_result(workspace_dir: Path, message: str) -> None:
    (workspace_dir / "result.txt").write_text(f"{message}\n", encoding="utf-8")
...
def _run_fallback(workspace_dir: Path, task: dict[str, Any]) -> str:
    ...
    _write_result(workspace_dir, message)
...
def main() -> int:
    ...
    task = _load_task(Path(args.task_json))
    ...
    elif os.environ.get("DEEPAGENT_MODE", "").strip().lower() == "real":
        _run_real_deepagent(workspace_dir, task)
    else:
        _run_fallback(workspace_dir, task)
    return 0
